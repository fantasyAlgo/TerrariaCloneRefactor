...
bool isCollisionTileHelper(unsigned char map[][settings::MAP_HEIGHT], int i, int j, int type){
    if (i < 0 || j < 0 || i >= settings::MAP_WIDTH || j >= settings::MAP_HEIGHT)
        return true;
...
bool isTileWall(int x, int y, ValueNoise1D noise){
  return y <= ((float)settings::MAP_HEIGHT/2 - (noise.eval(((float)x)/settings::NOISE_VARIABILITY))*settings::MAP_HEIGHT/4)+10;
}
Rectangle getTile(int x, int y, int type){
    if (type == TREE_BRANCH || type == TREE_TRUNK){ return {(float)x*22, (float)y*settings::BLOCK_CHUNK, settings::BLOCK_CHUNK, settings::BLOCK_CHUNK}; }
    return {(float)x*settings::BLOCK_CHUNK, (float)y*settings::BLOCK_CHUNK, settings::BLOCK_CHUNK, settings::BLOCK_CHUNK};
}
Rectangle getTileP(Vector2 pos, int type, int time){
    ...
    return {(float)pos.x*settings::BLOCK_CHUNK, (float)pos.y*settings::BLOCK_CHUNK, settings::BLOCK_CHUNK-1, settings::BLOCK_CHUNK-1};
}
Rectangle getTileWithSize(int x, int y, float size){
    return {(float)x*settings::BLOCK_CHUNK, (float)y*settings::BLOCK_CHUNK, size, size};
}
...
void removeUnSourcedWater(unsigned char map[][settings::MAP_HEIGHT], int iTile, int jTile, ValueNoise1D noise){
    if (!hasSource(map, iTile, jTile)){
        if (jTile > (settings::MAP_HEIGHT/2 - (noise.eval(((float)(iTile))/settings::NOISE_VARIABILITY))*settings::MAP_HEIGHT/4)+10)
